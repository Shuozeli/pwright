...
use pwright_integration_tests::{chrome_http_url, lightpanda_ws_url};

/// Helper to create a Lightpanda-backed page using a direct WS connection.
/// Lightpanda requires separate connections per tab, so we create a new
/// browser connection per tab. The tab is created via `new_tab("about:blank")`
/// and navigation is done separately via `goto` to avoid double-navigation.
async fn lightpanda_page(
    url: &str,
) -> (pwright_bridge::TabHandle, pwright_bridge::playwright::Page) {
    ...
    if url != "about:blank" {
        ...
        // Lightpanda may need time to settle after navigation
...
// Uses https://example.com instead. Also requires a new WS connection
// per tab (separate Browser::connect per tab).
// ═══════════════════════════════════════════════════════════════════

/// Open multiple tabs on Lightpanda (separate connections), close sequentially.
#[tokio::test]
#[ignore = "requires docker: lightpanda"]
async fn lightpanda_multiple_tabs_close_sequentially() {
    // Open 3 tabs — each needs its own browser connection on Lightpanda
    let (tab1, page1) = lightpanda_page("https://example.com").await;
    let (tab2, page2) = lightpanda_page("https://example.com").await;
    let (tab3, page3) = lightpanda_page("https://example.com").await;

    // Verify each page loaded (example.com has <h1>Example Domain</h1>)
    let text1: String = page1
        .evaluate_into("document.querySelector('h1').textContent")
        .await
        .unwrap();
...
#[tokio::test]
#[ignore = "requires docker: lightpanda"]
async fn lightpanda_navigate_to_google_com() {
    let (tab, page) = lightpanda_page("https://www.google.com").await;
...
/// Lightpanda: open multiple tabs with example.com, close in mixed order.
#[tokio::test]
#[ignore = "requires docker: lightpanda"]
async fn lightpanda_multiple_tabs_close_mixed_order() {
    let (tab1, page1) = lightpanda_page("https://example.com").await;
    let (tab2, page2) = lightpanda_page("https://example.com").await;
    let (tab3, page3) = lightpanda_page("https://example.com").await;
