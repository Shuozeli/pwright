//! Integration tests for evaluate_sync, evaluate_async, and typed variants.
//!
//! Tests cover: primitive types, complex objects, error handling, Promise
//! resolution, async fetch, DOM manipulation, and the boundary between
...
// ═══════════════════════════════════════════════════════════════════
//  evaluate_sync — synchronous expressions
// ═══════════════════════════════════════════════════════════════════
...
async fn sync_returns_undefined_as_no_value() {
    let page = connect_and_navigate("/content.html").await;
    let result = page.evaluate_sync("undefined").await.unwrap();
...
async fn sync_dom_element_count() {
    let page = connect_and_navigate("/content.html").await;
    let count: i64 = page
        .evaluate_sync_into("document.querySelectorAll('*').length")
        .await
        .unwrap();
...
async fn sync_dom_manipulation() {
    ...
    let text: String = page
        .evaluate_sync_into("document.getElementById('injected').textContent")
        .await
        .unwrap();
...
async fn sync_js_exception_returns_error() {
    let page = connect_and_navigate("/content.html").await;
    let result = page.evaluate_sync("throw new Error('boom')").await;
...
async fn sync_reference_error() {
    let page = connect_and_navigate("/content.html").await;
    let result = page.evaluate_sync("nonExistentVariable").await;
...
async fn sync_into_type_mismatch() {
    ...
    let result: Result<String, _> = page.evaluate_sync_into("42").await;
...
async fn sync_into_json_deserialize() {
    ...
    let point: FromEvalJson<Point> = page
        .evaluate_sync_into(r#"JSON.stringify({x: 1.5, y: 2.5})"#)
        .await
        .unwrap();
...
async fn sync_promise_returns_object_not_value() {
    ...
    let result = page
        .evaluate_sync("new Promise(resolve => resolve(42))")
        .await
        .unwrap();
...
// ═══════════════════════════════════════════════════════════════════
//  evaluate_async — Promise-awaiting expressions
// ═══════════════════════════════════════════════════════════════════
...
async fn async_resolves_promise() {
    let page = connect_and_navigate("/content.html").await;
    let result = page
        .evaluate_async("new Promise(resolve => resolve(42))")
        .await
        .unwrap();
...
async fn async_resolves_promise_string() {
    let page = connect_and_navigate("/content.html").await;
    let result = page
        .evaluate_async("new Promise(resolve => resolve('hello'))")
        .await
        .unwrap();
...
async fn async_resolves_delayed_promise() {
    let page = connect_and_navigate("/content.html").await;
    let result = page
        .evaluate_async("new Promise(resolve => setTimeout(() => resolve('delayed'), 100))")
        .await
        .unwrap();
...
async fn async_rejects_promise_returns_error() {
    let page = connect_and_navigate("/content.html").await;
    let result = page
        .evaluate_async("Promise.reject(new Error('rejected'))")
        .await;
...
async fn async_fetch_text() {
    ...
    let base = server_base_url();
    let result = page
        .evaluate_async(&format!("fetch('{base}/health').then(r => r.text())"))
        .await
        .unwrap();
...
async fn async_await_syntax() {
    ...
    let result = page
        .evaluate_async("(async () => { const x = await Promise.resolve(99); return x; })()")
        .await
        .unwrap();
...
async fn async_into_typed() {
    let page = connect_and_navigate("/content.html").await;
    let val: i64 = page
        .evaluate_async_into("Promise.resolve(123)")
        .await
        .unwrap();
    assert_eq!(val, 123);
}
...
async fn sync_and_async_agree_on_simple_expression() {
    ...
    let sync_result = page.evaluate_sync("2 + 2").await.unwrap();
    let async_result = page.evaluate_async("2 + 2").await.unwrap();
...
async fn async_resolves_where_sync_would_not() {
    ...
    let async_result = page
        .evaluate_async("Promise.resolve('resolved')")
        .await
        .unwrap();
    assert_eq!(async_result["value"], "resolved");

    // Sync returns the Promise object, not the resolved value
    let sync_result = page
        .evaluate_sync("Promise.resolve('resolved')")
        .await
        .unwrap();
    assert_ne!(
